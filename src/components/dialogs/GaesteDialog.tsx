/**
 * GaesteDialog — pre-generated create/edit dialog for Gaeste.
 *
 ...
 * GaesteDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Gaeste['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<GaesteDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Gaeste } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Gaeste';
...
/** Widened prefill type for GaesteDialog.defaultValues — see file header. */
export type GaesteDialogDefaults = Gaeste['fields'];
...
interface GaesteDialogProps {
  ...
  defaultValues?: GaesteDialogDefaults;
...
export function GaesteDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: GaesteDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Gaeste['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'gaeste');
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('gaeste') })
    : t('new_entity', { entity: appLabel('gaeste') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'vorname': (
      <div key="vorname" className="space-y-1.5">
        ...
        {showErrors && !fields.vorname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('gaeste', 'vorname')}</p>
        )}
      </div>
    ),
    'nachname': (
      <div key="nachname" className="space-y-1.5">
        ...
        {showErrors && !fields.nachname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('gaeste', 'nachname')}</p>
        )}
      </div>
    ),
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('gaeste', 'email')}</Label>
        <Input
          id="email"
          type="email"
          inputMode="email"
    ...
    'telefon': (
      <div key="telefon" className="space-y-1.5">
        <Label htmlFor="telefon">{fieldLabel('gaeste', 'telefon')}</Label>
        <Input
          id="telefon"
          type="tel"
          inputMode="tel"
          placeholder="z. B. 040 9876543"
    ...
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Spezialwünsche, Allergien, besondere Bedürfnisse..."
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail", "telefon": "Telefon", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "land": "Land", "notizen": "Notizen"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.GAESTE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
