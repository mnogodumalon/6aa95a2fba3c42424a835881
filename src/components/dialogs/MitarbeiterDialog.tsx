...
export function MitarbeiterDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitarbeiterDialogProps) {
      ...
      <div key="rolle" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.rolle) === 'reinigung'}
            onClick={() => setFields(f => ({ ...f, rolle: (lookupKey(f.rolle) === 'reinigung' ? undefined : 'reinigung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.rolle) === 'reinigung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('mitarbeiter', 'rolle', 'reinigung') ?? 'Reinigung'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.rolle) === 'vermietung'}
            onClick={() => setFields(f => ({ ...f, rolle: (lookupKey(f.rolle) === 'vermietung' ? undefined : 'vermietung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.rolle) === 'vermietung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('mitarbeiter', 'rolle', 'vermietung') ?? 'Vermietung'}
          </button>
        </div>
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('mitarbeiter', 'status', 'inaktiv') ?? 'Inaktiv'}
          </button>
        </div>
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "rolle": "Rolle", "telefon": "Telefon", "email": "E-Mail", "status": "Status"};
