/**
 * ReinigungenDialog — pre-generated create/edit dialog for Reinigungen.
 *
 ...
 * ReinigungenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Reinigungen['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<ReinigungenDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Reinigungen, Wohnungen, Buchungen, Mitarbeiter, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Reinigungen';
...
/** Widened prefill type for ReinigungenDialog.defaultValues — see file header. */
export type ReinigungenDialogDefaults = Omit<Reinigungen['fields'], 'status'> & {
    status?: LookupValue | string;
  };

interface ReinigungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Reinigungen['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: ReinigungenDialogDefaults;
...
export function ReinigungenDialog({ open, onClose, onSubmit, defaultValues, recordId, wohnungenList, buchungenList, mitarbeiterList, enablePhotoScan = true, enablePhotoLocation = true }: ReinigungenDialogProps) {
  const [fields, setFields] = useState<Partial<Reinigungen['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['wohnung', 'datum', 'reinigungskraft', 'status'] as const;
  ...
  const computedContext = useMemo<ComputedContext>(() => ({
    lookupLists: {
      ...
      'reinigungskraft': mitarbeiterList,
    },
  }), [wohnungenList, buchungenList, mitarbeiterList, ]);
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Reinigungen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'reinigungen');
      await onSubmit(clean as Reinigungen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="reinigungskraft" entity="Mitarbeiter">\n${JSON.stringify(mitarbeiterList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["wohnung", "buchung", "reinigungskraft"]);
        ...
        if (buchungName) {
          const buchungMatch = buchungenList.find(r => matchName(buchungName!, [String(r.fields.buchungsnummer ?? '')]));
          if (buchungMatch) merged['buchung'] = createRecordUrl(APP_IDS.BUCHUNGEN, buchungMatch.record_id);
        }
        const reinigungskraftName = raw['reinigungskraft'] as string | null;
        if (reinigungskraftName) {
          const reinigungskraftMatch = mitarbeiterList.find(r => matchName(reinigungskraftName!, [[r.fields.vorname ?? '', r.fields.nachname ?? ''].filter(Boolean).join(' ')]));
          if (reinigungskraftMatch) merged['reinigungskraft'] = createRecordUrl(APP_IDS.MITARBEITER, reinigungskraftMatch.record_id);
        }
        return merged as Partial<Reinigungen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('reinigungen') })
    : t('new_entity', { entity: appLabel('reinigungen') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'wohnung': (
      <div key="wohnung" className="space-y-1.5">
        ...
        {showErrors && !fields.wohnung && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('reinigungen', 'wohnung')}</p>
        )}
      </div>
    ),
    'buchung': (
    ...
    'datum': (
      <div key="datum" className="space-y-1.5">
        ...
        <DatePicker
          id="datum"
          placeholder="Wann ist der Reinigungstermin?"
          mode="date"
        ...
        {showErrors && !fields.datum && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('reinigungen', 'datum')}</p>
        )}
      </div>
    ),
    'reinigungskraft': (
      <div key="reinigungskraft" className="space-y-1.5">
        ...
        <Combobox
          id="reinigungskraft"
          placeholder="Reinigungskraft zuweisen"
          items={mitarbeiterListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.vorname ?? r.record_id),
          }))}
          value={extractRecordId(fields.reinigungskraft)}
          onChange={id => setFields(f => ({ ...f, reinigungskraft: id ? createRecordUrl(APP_IDS.MITARBEITER, id) : undefined }))}
          onCreateNew={(q) => openCreateMitarbeiter("reinigungskraft", q)}
          createLabel={t('create_in', { entity: appLabel('mitarbeiter') })}
        />
        {showErrors && !fields.reinigungskraft && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('reinigungen', 'reinigungskraft')}</p>
        )}
      </div>
    ),
    'status': (
      <div key="status" className="space-y-1.5">
        <Label htmlFor="status">{fieldLabel('reinigungen', 'status')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('reinigungen', 'status', 'offen') ?? 'Offen'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('reinigungen', 'status', 'erledigt') ?? 'Erledigt'}
          </button>
        </div>
        {showErrors && !fields.status && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('reinigungen', 'status')}</p>
        )}
      </div>
    ),
    'bemerkungen': (
      <div key="bemerkungen" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkungen"
          placeholder="Besonderheiten, Schäden, Notizen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"wohnung": "Wohnung", "buchung": "Buchung", "datum": "Reinigungsdatum", "reinigungskraft": "Reinigungskraft", "status": "Status", "bemerkungen": "Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"wohnung": {"name": "Name der Wohnung", "beschreibung": "Beschreibung", "stockwerk": "Stockwerk", "schlafplaetze": "Schlafplätze", "quadratmeter": "Quadratmeter", "grundpreis_pro_nacht": "Grundpreis pro Nacht (€)", "endreinigung_preis": "Endreinigung (€)", "ausstattung": "Ausstattung", "foto": "Foto", "status": "Status"}, "buchung": {"buchungsnummer": "Buchungsnummer", "wohnung": "Wohnung", "gast": "Gast", "anreise": "Anreise", "abreise": "Abreise", "anzahl_personen": "Anzahl Personen", "status": "Status", "gesamtpreis": "Gesamtpreis (€)", "anzahlung_erhalten": "Anzahlung erhalten", "notizen": "Notizen"}, "reinigungskraft": {"vorname": "Vorname", "nachname": "Nachname", "rolle": "Rolle", "telefon": "Telefon", "email": "E-Mail", "status": "Status"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.REINIGUNGEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createBuchungenOpen && (
      <BuchungenDialog
        ...
        defaultValues={createBuchungenInitial
          ? ({ buchungsnummer: createBuchungenInitial } as any)
          : undefined}
