...
import type { Mitarbeiter, Wohnungen, Gaeste, Buchungen, Reinigungen, CreateMitarbeiter, CreateWohnungen, CreateGaeste, CreateBuchungen, CreateReinigungen } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getWohnungen(): Promise<Wohnungen[]> {
    ...
    return hydrateRecords(records, 'wohnungen');
  }
  static async queryWohnungen(q: RecordQuery = {}): Promise<Wohnungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WOHNUNGEN}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Wohnungen[];
  ...
  static async countWohnungen(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.WOHNUNGEN}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  static async getWohnungenEntry(id: string): Promise<Wohnungen | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Wohnungen;
    return hydrateRecords([record], 'wohnungen')[0];
  }
  static async createWohnungenEntry(fields: CreateWohnungen): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.WOHNUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'wohnungen') });
    return { ...data, record_id: data.id };
  }
  static async updateWohnungenEntry(id: string, fields: Partial<CreateWohnungen>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.WOHNUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'wohnungen') });
    return { ...data, record_id: data.id };
  }
  ...
  static async getGaeste(): Promise<Gaeste[]> {
    ...
    return hydrateRecords(records, 'gaeste');
  }
  static async queryGaeste(q: RecordQuery = {}): Promise<Gaeste[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.GAESTE}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Gaeste[];
  ...
  static async countGaeste(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.GAESTE}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  static async getGaesteEntry(id: string): Promise<Gaeste | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Gaeste;
    return hydrateRecords([record], 'gaeste')[0];
  }
  static async createGaesteEntry(fields: CreateGaeste): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.GAESTE}/records`, { fields: cleanFieldsForApi(fields as any, 'gaeste') });
    return { ...data, record_id: data.id };
  }
  static async updateGaesteEntry(id: string, fields: Partial<CreateGaeste>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.GAESTE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'gaeste') });
    return { ...data, record_id: data.id };
  }
  ...
  // --- REINIGUNGEN ---
  static async getReinigungen(): Promise<Reinigungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.REINIGUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Reinigungen[];
    return hydrateRecords(records, 'reinigungen');
  }
  static async queryReinigungen(q: RecordQuery = {}): Promise<Reinigungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.REINIGUNGEN}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Reinigungen[];
  ...
  static async countReinigungen(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.REINIGUNGEN}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  static async getReinigungenEntry(id: string): Promise<Reinigungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.REINIGUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Reinigungen;
    return hydrateRecords([record], 'reinigungen')[0];
  }
  static async createReinigungenEntry(fields: CreateReinigungen): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.REINIGUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'reinigungen') });
    return { ...data, record_id: data.id };
  }
  static async updateReinigungenEntry(id: string, fields: Partial<CreateReinigungen>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.REINIGUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'reinigungen') });
    return { ...data, record_id: data.id };
  }
  static async deleteReinigungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.REINIGUNGEN}/records/${id}`);
  }
